...
# 리눅스를 새로 세팅하고나서, 초기에 설치해야할 패키지 목록
apt_requirements    = [
    'curl',
    'git',
    'python3-dev',
...
def _install_apt_requirements(apt_requirements):
    reqs = ''
    # 설치할 패키지들을 한칸씩 띄어서 하나의 문자열로 만들고 있다.
...
def _make_virtualenv():
    ...
    if not exists('~/.virtualenvs'):
        # 리눅스 상에서 환경변수를 세팅하는 명령어 (여러줄, 문자열로 여러줄 구성)
        ...
        # 위에서 작성한 스크립트(패스)를 .bashre라는 파일에 추가하시오
...
# 연결에 필요한 아파치 서버의 환경설정값
# wsgi.py가 엔트리 포인트라는 부분, 포트가 80번 (기본 http)
# 접속 로그, 에러 로그 위치 지정
...
# 프로젝트 폴더 및 아파치 연동된 부분에 관련되어 권한 조정
# 리눅스 상에서 누구나 파일/다렉토리 상에서 엑세스 할수 있게(수정포함) 조정
# chmod 777 파일
def _grant_apache2():
...
# 크게 흐름을 이해한다.
# git 생성
# 개발 , 개발된 코드는 git에 최신소스 반영
# 최초 세팅 및 배포
# fab new_server (계속 반복)
    # 추가개발
